...
class SpecWriter(BaseAgent):
    ...
    async def initialize_spec(self) -> AgentResponse:
        response = await self.ask_question(
            ...
            allow_empty=False,
            buttons={
                # FIXME: must be lowercase because VSCode doesn't recognize it otherwise. Needs a fix in the extension
                "continue": "continue",
    ...
    async def update_spec(self, iteration_mode) -> AgentResponse:
        ...
        llm_response: str = await llm(convo, parser=StringParser())
