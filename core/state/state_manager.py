...
class StateManager:
    ...
    should use the `StateManager.current` attribute. All changes
    ...
    async def create_project(self, name: str, folder_name: Optional[str] = None) -> Project:
        ...
        :param folder_name: Folder name for the project (optional).
        ...
        # even for a new project, e.g., offline changes check and stats updating
    ...
    async def log_llm_request(self, request_log: LLMRequestLog, agent: Optional["BaseAgent"] = None):
        ...
        :param agent: The agent making the request (optional).
        """
    ...
    async def log_command_run(self, exec_log: ExecLogData):
        ...
        depends on the current state, it makes it easier to analyze the
        database by just looking at a single project state later.
    ...
    async def log_event(self, type: str, **kwargs):
        ...
        * loop detected
        """
        # TODO: implement this
        # Consider setting this/orchestrator so that the side effect is to send
        # the update to the UI (e.g., VSCode extension)
    ...
    async def save_file(
        ...
        the database it may be rolled back if `next_state` is never committed.

        :param path: The file path.
        :param content: The file content.
        :param metadata: Optional metadata (e.g., description) to save with the file.
    ...
    async def import_files(self) -> tuple[list[File], list[File]]:
        ...
        to the database until the new state is committed.
    ...
    def workspace_is_empty(self) -> bool:
        """
        Returns whether the workspace has any files in it or is empty.
        """
        return not bool(self.file_system.list())

    async def get_application_logs(self):
        """
        Automatic log collection from the application log file.
        """
        try:
            config = get_config()
            log_path = config.log.output  # Use the configured log output path
            with open(log_path, 'r') as log_file:
                logs = log_file.read()
            return logs
        except Exception as e:
            log.error(f"Failed to read application logs: {e}")
            return None
